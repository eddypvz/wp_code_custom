    ...
    public function Postype($slug = "", $label = "", $args = []) {
        ...
        if (!$args["skip_post_type_register"]) {
            ...
            add_action( 'init', function () use ($args) {
                ...
                if (!empty($args["label"])) $p_object->label = $args["label"];
                if (!empty($args["label"])) $p_object->labels->name = $args["label"];
                if (!empty($args["label"])) $p_object->labels->menu_name = $args["label"];
                if (!empty($args["labelSingular"])) $p_object->labels->singular_name = $args["labelSingular"];
                if (!empty($args["icon"])) $p_object->menu_icon = $args["icon"];
                return true;
            });
        }
